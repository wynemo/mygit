...
class CommitDetailView(QTextBrowser):
    """
    Commit 详细信息视图
    直接使用 QTextEdit 作为主控件，显示选中 commit 的详细信息
    """
    ...
    def __init__(self, parent=None):
        ...
        self.git_manager = None  # 存储 Git 仓库管理器实例
        self.setStyleSheet(
            """
    ...
    def update_commit_detail(self, git_manager, commit):
        ...
        if not self.current_commit:
            self.clear()  # 如果没有 commit 对象，则清空视图
            return

        try:
            # 获取并存储当前 commit 关联的所有分支信息
            self.all_branches = self.get_commit_branches(self.git_manager, self.current_commit)
            # 调用内部方法来渲染提交详情的 HTML 内容
            self._render_commit_detail()

        except Exception as e:
            self.clear()
            # 使用 setHtml 以防错误信息包含可能被错误解析的特殊字符
            self.setHtml(f"获取 commit 详细信息失败：{e!s}")

    def _render_commit_detail(self):
        """
        根据当前存储的提交信息 (self.current_commit, self.all_branches) 和
        分支展开状态 (self.branches_expanded) 来渲染提交详情的 HTML 内容。
        此方法负责生成最终在视图中显示的 HTML 字符串。
        """
        ...
        if num_branches > 0:  # 仅当存在关联分支时才处理分支显示
            # 情况 1: 分支数量超过最大显示限制 (MAX_BRANCHES_TO_SHOW) 且当前未展开
            ...
            # 情况 2: 分支列表已展开 (self.branches_expanded is True)
            ...
            # 情况 3: 分支数量未超过最大显示限制，或等于限制
            else:
                # 直接显示所有分支，无需展开/折叠功能
                branch_text = f"In {num_branches} branches: {', '.join(self.all_branches)}"

        # --- 组合所有信息为 HTML ---
        # 使用 <pre> 标签来保留提交消息的原始格式 (如空格和换行)，同时允许 CSS 控制自动换行 (white-space: pre-wrap; word-wrap: break-word;)
